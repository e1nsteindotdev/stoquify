  ...
  return (
    <div className={`flex flex-col lg:flex-row w-full lg:border-b-1 lg:border-white ${index % 2 === 0 && "justify-end border-r-1"}`}>
      <div className="flex justify-center h-[230px] xs:h-auto gap-3 lg:gap-5 xs:px-3 my-5 lg:px-5">
        {/* left side  */}
        <div className={`flex flex-col gap-1.5 lg:gap-3 min-w-[135px] flex-1 ${index % 2 === 0 ? "order-1 items-end" : "order-2 item-start"}`}>
          ...
          <div className="flex flex-1 min-h-0 overflow-hidden flex-col justify-between gap-2 h-full">
            ...
            >
              {products?.map((p, i) => p.categoryId === category._id && <p
                className={`text-black text-wrap transition-all duration-400 ease-out ${selectedProduct?._id === p._id ? 'opacity-100' : "opacity-30"}`}
              >{p.title}</p>)}
            </div>
          </div>
        ...
        {/* right side */}
        <div className={`relative min-w-[185px] min-h-[230px] flex-2 aspect-[4/5] border-1 border-white ${index % 2 === 0 ? "order-2" : "order-1"}`}>
          {products?.map((p) => (
            <img key={p._id}
