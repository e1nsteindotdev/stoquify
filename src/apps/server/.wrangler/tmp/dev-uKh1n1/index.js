...
function getCauseFromUnknown(cause3) {
  ...
  if (isObject(cause3)) return Object.assign(new UnknownCauseError(), cause3);
  return void 0;
}
...
var PgColumnBuilder = class extends ColumnBuilder {
  ...
  array(size10) {
    return new PgArrayBuilder(this.config.name, this, size10);
  }
...
var PgArrayBuilder = class extends PgColumnBuilder {
  ...
  constructor(name, baseBuilder, size10) {
    super(name, "array", "PgArray");
    this.config.baseBuilder = baseBuilder;
    this.config.size = size10;
  }
  /** @internal */
  build(table) {
...
var SQL = class _SQL {
  ...
  toQuery(config) {
    return tracer.startActiveSpan("drizzle.buildSQL", (span4) => {
...
function mapUpdateSet(table, values3) {
  const entries2 = Object.entries(values3).filter(([, value]) => value !== void 0).map(([key, value]) => {
...
function inArray(column, values3) {
  if (Array.isArray(values3)) {
    ...
    return sql`${column} in ${values3.map((v) => bindIfParam(v, column))}`;
  }
  return sql`${column} in ${bindIfParam(values3, column)}`;
}
...
function notInArray(column, values3) {
  if (Array.isArray(values3)) {
    ...
    return sql`${column} not in ${values3.map((v) => bindIfParam(v, column))}`;
  }
  return sql`${column} not in ${bindIfParam(values3, column)}`;
}
...
function between(column, min3, max5) {
  return sql`${column} between ${bindIfParam(min3, column)} and ${bindIfParam(
    max5,
    column
  )}`;
}
...
var SQLiteDialect = class {
  ...
  buildUpdateSet(table, set6) {
    const tableColumns = table[Table.Symbol.Columns];
    const columnNames = Object.keys(tableColumns).filter(
      (colName) => set6[colName] !== void 0 || tableColumns[colName]?.onUpdateFn !== void 0
    );
    const setSize = columnNames.length;
    return sql.join(columnNames.flatMap((colName, i) => {
      const col = tableColumns[colName];
      const onUpdateFnResult = col.onUpdateFn?.();
      const value = set6[colName] ?? (is(onUpdateFnResult, SQL) ? onUpdateFnResult : sql.param(onUpdateFnResult, col));
  ...
  buildUpdateQuery({ table, set: set6, where, returning, withList, joins, from, limit, orderBy }) {
    const withSql = this.buildWithCTE(withList);
    const setSql = this.buildUpdateSet(table, set6);
    ...
    const chunks = fields.flatMap(({ field }, i) => {
      ...
      if (is(field, SQL.Aliased) && field.isSelectionField) {
        ...
        if (field.columnType === "SQLiteNumericBigInt") {
          if (isSingleTable) {
            chunk2.push(sql`cast(${sql.identifier(this.casing.getColumnCasing(field))} as text)`);
          } else {
            chunk2.push(
          ...
          if (isSingleTable) {
            ...
            chunk2.push(sql`${sql.identifier(tableName)}.${sql.identifier(this.casing.getColumnCasing(field))}`);
          }
        }
      } else if (is(field, Subquery)) {
        ...
        if (is(singleOrderBy, SQLiteColumn)) {
          ...
          for (let i = 0; i < singleOrderBy.queryChunks.length; i++) {
            ...
            if (is(chunk2, SQLiteColumn)) {
              singleOrderBy.queryChunks[i] = sql.identifier(this.casing.getColumnCasing(chunk2));
            }
          }
  ...
  buildInsertQuery({ table, values: valuesOrSelect, onConflict, returning, withList, select }) {
    ...
    if (select) {
      ...
      for (const [valueIndex, value] of values3.entries()) {
        const valueList = [];
...
var SQLiteInsertBase = class extends QueryPromise {
  ...
  constructor(table, values3, session, dialect, withList, select) {
    super();
    this.session = session;
    this.dialect = dialect;
    this.config = { table, values: values3, withList, select };
  }
...
var SQLiteUpdateBase = class extends QueryPromise {
  ...
  constructor(table, set6, session, dialect, withList) {
    super();
    this.session = session;
    this.dialect = dialect;
    this.config = { set: set6, table, withList, joins: [] };
  }
...
var BaseSQLiteDatabase = class {
  ...
  with(...queries) {
    ...
    return { select, selectDistinct, update: update5, insert: insert3, delete: delete_3 };
  }
  select(fields) {
...
var isFunction2 = /* @__PURE__ */ __name((input) => typeof input === "function", "isFunction");
...
var isObject2 = /* @__PURE__ */ __name((input) => isRecordOrArray(input) || isFunction3(input), "isObject");
...
var isPromiseLike = /* @__PURE__ */ __name((input) => hasProperty(input, "then") && isFunction3(input.then), "isPromiseLike");
...
var isGeneratorFunction = /* @__PURE__ */ __name((u) => isObject2(u) && u.constructor === genConstructor, "isGeneratorFunction");
...
var flatten = /* @__PURE__ */ flatMap2(identity2);
...
var getSomes = /* @__PURE__ */ filterMap(identity2);
...
var union3 = /* @__PURE__ */ dual(2, (self, that) => mutate2(empty6(), (set6) => {
...
var flatten3 = /* @__PURE__ */ __name((self) => flatMap6(self, identity2), "flatten");
...
var prettyErrorMessage = /* @__PURE__ */ __name((u) => {
  ...
  try {
    if (hasProperty(u, "toString") && isFunction3(u["toString"]) && u["toString"] !== Object.prototype.toString && u["toString"] !== globalThis.Array.prototype.toString) {
      return u["toString"]();
    }
  } catch {
  }
...
var die2 = /* @__PURE__ */ __name((defect) => isObject2(defect) && !(spanSymbol in defect) ? withFiberRuntime((fiber) => failCause(die(capture(defect, currentSpanFromFiber(fiber))))) : failCause(die(defect)), "die");
...
var fail2 = /* @__PURE__ */ __name((error) => isObject2(error) && !(spanSymbol in error) ? withFiberRuntime((fiber) => failCause(fail(capture(error, currentSpanFromFiber(fiber))))) : failCause(fail(error)), "fail");
...
var flatten4 = /* @__PURE__ */ __name((self) => flatMap7(self, identity2), "flatten");
...
var tap = /* @__PURE__ */ dual((args2) => args2.length === 3 || args2.length === 2 && !(isObject2(args2[1]) && "onlyEffect" in args2[1]), (self, f) => flatMap7(self, (a) => {
  const b = typeof f === "function" ? f(a) : f;
  if (isEffect(b)) {
    return as(b, a);
  } else if (isPromiseLike(b)) {
...
var exitFlatten = /* @__PURE__ */ __name((self) => pipe(self, exitFlatMap(identity2)), "exitFlatten");
...
var splitPathString = /* @__PURE__ */ __name((text2, delim) => {
  const split = text2.split(new RegExp(`\\s*${escape(delim)}\\s*`));
  return split;
}, "splitPathString");
var parsePrimitive = /* @__PURE__ */ __name((text2, path, primitive, delimiter, split) => {
  if (!split) {
...
var filterMap3 = /* @__PURE__ */ dual(2, (elements, pf) => map8(forEachSequential(elements, identity2), filterMap(pf)));
...
var memoize = /* @__PURE__ */ __name((self) => pipe(deferredMake(), flatMap7((deferred) => pipe(diffFiberRefsAndRuntimeFlags(self), intoDeferred(deferred), once2, map8((complete3) => zipRight(complete3, pipe(deferredAwait(deferred), flatMap7(([patch9, a]) => as(zip2(patchFiberRefs(patch9[0]), updateRuntimeFlags(patch9[1])), a)))))))), "memoize");
...
var once2 = /* @__PURE__ */ __name((self) => map8(make27(true), (ref) => asVoid(whenEffect(self, getAndSet2(ref, false)))), "once");
...
var unsafeMakeSpan = /* @__PURE__ */ __name((fiber, name, options) => {
  ...
  if (disablePropagation) {
    ...
    span4 = tracer4.span(name, parent, options.context ?? empty3(), links, timingEnabled ? clock3.unsafeCurrentTimeNanos() : bigint02, options.kind ?? "internal", options);
...
var taggedWithLabels2 = /* @__PURE__ */ dual(2, (self, extraTags) => {
  return make31(self.keyType, (input, extraTags1) => self.unsafeUpdate(input, union2(extraTags, extraTags1)), (extraTags1) => self.unsafeValue(union2(extraTags, extraTags1)), (input, extraTags1) => self.unsafeModify(input, union2(extraTags, extraTags1)));
});
...
var allValidate = /* @__PURE__ */ __name((effects, reconcile, options) => {
  ...
  return flatMap7(forEach7(eitherEffects, identity2, {
...
var allEither = /* @__PURE__ */ __name((effects, reconcile, options) => {
  ...
  if (options?.discard) {
    return forEach7(eitherEffects, identity2, {
      concurrency: options?.concurrency,
      batching: options?.batching,
      discard: true,
      concurrentFinalizers: options?.concurrentFinalizers
  ...
  return map8(forEach7(eitherEffects, identity2, {
    concurrency: options?.concurrency,
    batching: options?.batching,
    concurrentFinalizers: options?.concurrentFinalizers
...
var all2 = /* @__PURE__ */ __name((arg, options) => {
  ...
  return options?.discard !== true && reconcile._tag === "Some" ? map8(forEach7(effects, identity2, options), reconcile.value) : forEach7(effects, identity2, options);
}, "all");
...
var mergeAll3 = /* @__PURE__ */ dual((args2) => isFunction3(args2[2]), (elements, zero2, f, options) => matchSimple(options?.concurrency, () => fromIterable(elements).reduce((acc, a, i) => zipWith2(acc, a, (acc2, a2) => f(acc2, a2, i)), succeed(zero2)), () => flatMap7(make27(zero2), (acc) => flatMap7(forEach7(elements, (effect2, i) => flatMap7(effect2, (a) => update3(acc, (b) => f(b, a, i))), options), () => get12(acc)))));
var partition3 = /* @__PURE__ */ dual((args2) => isIterable(args2[0]), (elements, f, options) => pipe(forEach7(elements, (a, i) => either2(f(a, i)), options), map8((chunk2) => partitionMap2(chunk2, identity2))));
...
var intersect2 = /* @__PURE__ */ dual(2, (self, that) => {
...
var intersect4 = /* @__PURE__ */ dual(2, (self, that) => intersectLoop(self.intervals, that.intervals, empty4()));
var intersectLoop = /* @__PURE__ */ __name((_left, _right, _acc) => {
  let left3 = _left;
  let right3 = _right;
  let acc = _acc;
  while (isNonEmpty(left3) && isNonEmpty(right3)) {
    const interval = pipe(headNonEmpty2(left3), intersect3(headNonEmpty2(right3)));
...
var cachedFunction = /* @__PURE__ */ __name((f, eq2) => {
  return pipe(sync(() => empty17()), flatMap7(makeSynchronized), map8((ref) => (a) => pipe(ref.modifyEffect((map14) => {
    const result = pipe(map14, get8(new Key(a, eq2)));
    if (isNone2(result)) {
      return pipe(deferredMake(), tap((deferred) => pipe(diffFiberRefs(f(a)), intoDeferred(deferred), fork)), map8((deferred) => [deferred, pipe(map14, set4(new Key(a, eq2), deferred))]));
    }
...
var intersect6 = /* @__PURE__ */ dual(2, (self, that) => intersectWith(self, that, intersect5));
...
var repeat_combined = /* @__PURE__ */ dual(2, (self, options) => {
  ...
  const withTimes = options.times ? intersect6(withUntil, recurs(options.times)).pipe(map11((intersectionPair) => intersectionPair[0])) : withUntil;
...
var once3 = /* @__PURE__ */ asVoid2(/* @__PURE__ */ recurs(1));
...
function wrapProcedures(record, runtime5, parentPath, contextTag) {
  const result = {};
    ...
    if (isEffectProcedure(value)) {
      ...
      if (_tag === "query") {
        result[key] = _builder.query((opts) => {
          let effect2 = Effect_exports.withSpan(currentPath, { captureStackTrace: _captureStackTrace })(tracedResolver({
            ctx: opts.ctx,
            input: opts.input,
            signal: opts.signal
          }));
          if (contextTag) {
            effect2 = Effect_exports.provideService(effect2, contextTag, opts.ctx);
          }
      ...
      result[key] = wrapProcedures(value, runtime5, currentPath, contextTag);
    } else {
      result[key] = value;
    }
  }
...
var EffectTRPCBuilder = class _EffectTRPCBuilder {
   ...
   */
  create(opts) {
    const { runtime: runtime5, contextTag, ...trpcOptions } = opts;
    const trpc = initTRPC.create(trpcOptions);
    return {
      procedure: trpc.procedure,
      effect: makeEffectProcedure(trpc.procedure),
      router: /* @__PURE__ */ __name((procedures) => {
        const wrappedProcedures = wrapProcedures(procedures, runtime5, "", contextTag);
...
var UserServiceLive = Layer_exports.succeed(UserService, {
  findById: /* @__PURE__ */ __name((id) => Effect_exports.succeed(users2.find((u) => u.id === id)), "findById"),
  list: /* @__PURE__ */ __name(() => Effect_exports.succeed(users2), "list"),
  create: /* @__PURE__ */ __name((name) => Effect_exports.succeed({ id: String(crypto.randomUUID()), name }), "create")
});

// src/services/my-service.ts
var MyService = class extends Context_exports.Tag("MyService")() {
  static {
    __name(this, "MyService");
  }
};

// src/trpc.ts
var TRPCContext = class extends Context_exports.Tag("TRPCContext")() {
  static {
    __name(this, "TRPCContext");
  }
};
var D1Live = Layer_exports.succeed(D1, {});
var runtime4 = ManagedRuntime_exports.make(UserServiceLive.pipe(
  Layer_exports.merge(D1Live)
  // Layer.merge(MyServiceLive)
));
var t = initEffectTRPC.context().create({
  runtime: runtime4,
...
var usersRouter = {
  ...
  list: t.effect.query(function* () {
    ...
    const dbTest = Effect_exports.tryPromise({
      try: /* @__PURE__ */ __name(() => db.select().from(users).all(), "try"),
      catch: /* @__PURE__ */ __name(() => console.log("got error getting users"), "catch")
    });
    Effect_exports.runPromise(dbTest);
...
// .wrangler/tmp/bundle-SHTwuP/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-SHTwuP/middleware-loader.entry.ts
