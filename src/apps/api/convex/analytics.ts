...
export const getSalesData = query({
  ...
  handler: async (ctx, { period }) => {
    ...
    // Calculate online sales profit (subTotalCost for confirmed orders)
    const onlineProfit = filteredOrders.reduce(
      (sum, order) => sum + order.subTotalCost,
      0
    );

    // Calculate POS sales profit
    const posProfit = sales.reduce(
      (sum, sale) => sum + sale.subTotalCost,
      0
    );
...
export const getSalesByTimePeriod = query({
  ...
  handler: async (ctx, { period }) => {
    ...
    filteredOrders.forEach((order) => {
      ...
      existing.online += order.subTotalCost;
    ...
    sales.forEach((sale) => {
      ...
      const existing = dataMap.get(key) || { online: 0, pos: 0, onlineOrders: 0, posSales: 0, date: key };
      existing.pos += sale.subTotalCost;
      existing.posSales += 1;
      dataMap.set(key, existing);
    });
