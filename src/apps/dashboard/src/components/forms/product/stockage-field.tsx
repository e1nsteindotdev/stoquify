...
import {
  generateVIFingerPrint,
  mapsDeepEqual,
  type VariantElement as TVariant,
  type TVariantsInventory,
...
export default function StockageField({ variants, strat }: Props) {
  ...
  if (strat === "by_variants")
    ...
    return <ByNumberForm field={field} inventoryVariants={inventoryVariants} />;
...
function QuantityInput({
  ...
  fp: string;
  changeQuantity: (a: string, b: number) => void;
  path: string[];
}) {
  return (
    <div className="flex gap-2 items-center">
      <p className="text-[14px] text-neutral-700">Quantité</p>
      <Input
        onChange={(e) => changeQuantity(fp, Number(e.target.value))}
        placeholder="eg. 4"
...
function ByVariantsForm({ variants, inventoryVariants, field }: any) {
  const changeQuantity = function changeQuantity(fp: string, quantity: number) {
    field.setValue((prev) => {
      ...
      if (prevContent) prev.set(fp, { ...prevContent, quantity });
      return prev;
    });
  };
    ...
    function nest(result: string[], depth: number) {
      if (depth <= variants.length - 1)
        return variants[depth].options.map((o) => {
          return nest([...result, o], depth + 1);
        });
      else {
        const fp = generateVIFingerPrint(result);
        data.set(fp, {
          quantity: inventoryVariants.get(fp)?.quantity ?? 0,
          path: result,
        });
        return result;
      }
  ...
  const paths = useMemo(() => {
    ...
    function gen_paths(result: string[], depth: number) {
      if (depth < variants.length - 1)
        return variants[depth].options.map((o) => {
          return gen_paths([...result, o], depth + 1);
        });
      else {
        paths.push(result);
        return result;
    ...
    <div className="space-y-3 ">
      {variants.length > 0 ? (
        paths.map((path, j) => (
          ...
          >
            {variants.length > 1 ? (
              path.map((leaf: string, q) => (
                <div key={q}>
                  <div
                    className={cn("flex-1 py-3")}
                    style={{ paddingLeft: `${(q + 1) * basePadding}px` }}
                  >
              ...
              <div>
                <div
                  className="flex items-center flex-1 py-3 gap-10"
                  style={{ paddingLeft: `${variants.length * basePadding}px` }}
                >
                  <LittleItem key={path[0]}>{path[0]}</LittleItem>
                  <QuantityInput
                    fp={generateVIFingerPrint([...path])}
                    initialQuantity={
                      data.get(generateVIFingerPrint([...path]))?.quantity ?? 0
                    }
            ...
            {variants.length > 1 &&
              lastOption?.map((p, q) => {
                const fp = generateVIFingerPrint([...path, p]);
                return (
                  <div key={fp}>
                    <div
                      className="flex items-center flex-1 py-3 gap-10"
                      style={{
                        paddingLeft: `${variants.length * basePadding}px`,
                      }}
                    >
                      <LittleItem>{p}</LittleItem>
                      <QuantityInput
                        fp={generateVIFingerPrint([...path, p])}
                        initialQuantity={
                          data.get(generateVIFingerPrint([...path, p]))
                            ?.quantity ?? 0
                        }
                        changeQuantity={changeQuantity}
                        path={[...path, p]}
